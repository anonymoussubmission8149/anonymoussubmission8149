if True:#For code folding
    ...
    ITERSCHEDULERS = ["OneCycleLR","CyclicLR","ReduceLROnPlateau"] #torch lr schedulers which should be called at each iteration, not each epoch (not exhaustive!)
    STEPSSCEDULERS = ["OneCycleLR","CyclicLR"] #replace total steps argument with epochs * n_batches
...
class AdaCapNet(BaseEstimator, metaclass=ABCMeta):
    @abstractmethod
    ...
    def _initialize_training(self, data_length):
        ...
        if type(self.batch_size) in [int] and not self.batch_learning:
            if self.batch_size > MAX_GPU_MATRIX_WIDTH and device == "cuda": self.batch_learning = True
        if self.epochs and self.batch_learning:
            batch_length, n_batches = self._initialize_batch(data_length)
            self.total_steps = int(self.max_iter * n_batches)
        else:
            self.total_steps = self.max_iter
        ...
        self.torch_seed = int(self._random_state.uniform(0,2**31))
        if device != "cpu":
            torch.cuda.manual_seed_all(self.torch_seed) #Otherwise weights initialized without fix seed
            self.hidden_layers = self.hidden_nn(**self.hidden_params)
            torch.manual_seed(self.torch_seed)#Otherwise dropout not deterministic
            torch.cuda.manual_seed(self.torch_seed) #Otherwise dropout not deterministic
            torch.backends.cudnn.deterministic = True #Otherwise dropout not deterministic
        else:
            torch.manual_seed(self.torch_seed)#Otherwise dropout not deterministic
            self.hidden_layers = self.hidden_nn(**self.hidden_params)
        ...
        if not self.cst_lr:
            self.update_lr_every_step = (self.batch_learning == False) or self.lr_scheduler in ITERSCHEDULERS
            updated_lr_scheduler_params = {**self.lr_scheduler_params}
            if self.lr_scheduler in STEPSSCEDULERS and "total_steps" in self.lr_scheduler_params.keys():
                updated_lr_scheduler_params["total_steps"] = self.total_steps
            
            self.lr_scheduler_instance = getattr(torch.optim.lr_scheduler, self.lr_scheduler)(self.optimizer_instance, **updated_lr_scheduler_params)
        del params  
    ...
    def _initialize_batch(self, n_samples):
        if type(self.batch_size) == float:
            batch_length = min(MAX_GPU_MATRIX_WIDTH, int(n_samples * self.batch_size))
        elif type(self.batch_size) == int:
            batch_length = min(MAX_GPU_MATRIX_WIDTH, self.batch_size, n_samples)
        elif n_samples > MAX_GPU_MATRIX_WIDTH:
            batch_length = int(MAX_GPU_MATRIX_WIDTH)
        n_batches = int(n_samples / batch_length)
        return batch_length, n_batches
        
    def _initialize_data(self, X, y):
        if self.batch_learning:
            self.batch_length, self.n_batches = self._initialize_batch(len(y))
    ...
    def _backward_propagate(self, loss):
        ...
        if not self.cst_lr:
            if self.update_lr_every_step or len(self.batches) == 0:
    ...
    def _check_termination(self):
        if self.current_iter == 0:
            ...
            return self._check_convergence() or self._check_divergence() or self._check_timeout() or self.current_iter >= self.total_steps
...
class AdaCapNetRegressor(RegressorMixin, AdaCapNet):
    def __init__(self,
        hidden_nn,
                *,
        hidden_params = {},
        ...
        batch_size  = batch_size, max_iter  = max_iter, epochs = epochs, max_runtime  = max_runtime, validation_fraction  = validation_fraction,
...
class AdaCapNetClassifier(ClassifierMixin, AdaCapNet):
    def __init__(self, 
        hidden_nn,
                *,
        hidden_params = {},
